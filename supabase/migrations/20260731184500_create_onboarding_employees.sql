-- HTL Factory onboarding child table: onboarding_employees
...
CREATE INDEX IF NOT EXISTS onboarding_employees_case_idx
  ON public.onboarding_employees (onboarding_case_id);

CREATE INDEX IF NOT EXISTS onboarding_employees_email_norm_idx
  ON public.onboarding_employees (email_normalized);

ALTER TABLE public.onboarding_employees ENABLE ROW LEVEL SECURITY;

COMMENT ON TABLE public.onboarding_employees IS
  'Repeatable employee CRM access requests. Not storage form2. Dedupe on (client_id, email_normalized). RLS enabled; zero permissive policies (service/Postgres only).';
