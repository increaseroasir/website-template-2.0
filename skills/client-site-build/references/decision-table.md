...
| Form submits rejected: "Please complete the security check" | Verify template head ≥ `3e2fadd` (WTV-017) and every `.cf-turnstile` has `data-sitekey`; re-hydrate + redeploy | A widget without sitekey never issues a token; with `TURNSTILE_SECRET_KEY` set the server rejects 100% of native-form leads while looking fine visually |
