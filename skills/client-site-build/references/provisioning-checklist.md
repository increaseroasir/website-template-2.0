...
| 6 | Set CF vars/secrets: `GHL_API_TOKEN`, `GHL_LOCATION_ID`, `TURNSTILE_SECRET_KEY`, `META_PIXEL_ID`, `META_CAPI_ACCESS_TOKEN`, `META_OFFLINE_WEBHOOK_SECRET`, `ADMIN_PASSWORD`, plus env vars `GOOGLE_SHEETS_ID` and `GHL_BASE_TAGS=new-lead` — all on BOTH Production and Preview | Secrets checklist in WIRING (values never pasted); `GHL_BASE_TAGS` must be `new-lead` — the snapshot intake workflow triggers on that exact tag (website↔snapshot handshake) |
