...
USAGE
  ...
  node skills/client-site-build/scripts/new-client.mjs --validate <name> [--verbose]
...
if (mode === '--init') {
  ...
  for (const [src, dst] of pairs) {
    const srcPath = join(skillRoot, 'assets', src);
    const dstPath = join(clientDir, dst);
    if (!existsSync(srcPath)) die(`Skill asset missing: ${srcPath}`, 2);
