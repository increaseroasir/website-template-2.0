...
### [TVD-040] Gate rules encode intent, not counts — preload is enforced by role
- **Date:** 2026-07-29
...
- **Decision:** Check 6 is enforced by role. Exactly the two intended preloads are permitted — one `as="image"` that must reference the LCP hero, one `as="style"` that must be the fonts stylesheet with both an `onload` promotion and a matching `<noscript>` fallback — and every other preload still fails. The owner ruling is preserved in substance and tightened in effect.
