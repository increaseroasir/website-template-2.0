...
### [WTV-041] Mobile Performance 62 was a cold Cloudflare edge, not a template defect — and four "obvious" fixes make it worse
- **Date:** 2026-07-29
...
- **Root cause — proven, not inferred:** A new Pages deployment hash has an **empty edge cache**, and the LCP element is the hero image, so the first request for it travels to origin. On one unchanged staging URL, one machine, identical settings: **cold → Performance 89, LCP 3.72s; warm run 2 → 99, LCP 1.73s; warm run 3 → 99, LCP 1.71s.** Deploy-then-immediately-audit scores a CDN cold start. A slow runner compounds it: holding the URL fixed and only raising CPU throttling walked the score 99 → 97 → 93 → 89 at 1×/8×/12×/16×. Cold edge on a slow sandbox reaches 62 without a single line of template code being at fault.
- **Fix:** `scripts/lighthouse-check.mjs` now primes the edge (document + every same-origin subresource) before scoring, runs 3 times and reports the **median**, prints the runner's `benchmarkIndex`, and keeps `--no-warm` for deliberately measuring a cold start. On the exact hash that was reported as 62, it now reports **Performance 99, Accessibility 100, Best-practices 100**, SEO exempt.
- **Four fixes that were tried and are measurably HARMFUL — do not apply them:**
  1. **`fetchpriority="high"` on the hero `<link rel="preload">`.** Lighthouse's own `lcp-discovery-insight` explicitly asks for this. It is wrong for this layout: it promotes the hero above the render-blocking stylesheet, and the hero cannot paint until that CSS applies. **LCP 2.52s → 3.01s across 3 runs.**
  2. **Inlining `home.css`.** Best FCP of any arm (1.59s) and catastrophic LCP: **2.5s → 6.5s, score 74-76.**
  3. **Removing the hero's `blur(72px)` steam blobs and the three above-the-fold `backdrop-filter` surfaces.** The theory was that animated blur behind a backdrop-filter forces a re-readback every frame. Plausible and false: Style & Layout and Rendering moved less than 3%. The design cost would have bought nothing.
  4. **Halving the hero image (88KB → 41KB, same dimensions).** No measurable change. The hero is *upscaled* on a 412px viewport (1600×1200 landscape covering a 412×947 portrait box), so it is not oversized, and its bytes are not the LCP constraint.
- **Rule / prevention:** Never gate on a single Lighthouse run against a fresh deployment hash. Warm first, take a median, and record `benchmarkIndex` before calling a number a defect. When a performance hypothesis is cheap to test, test it — three of the four above were confident, reasonable, and wrong.
- **Status:** FIXED (measurement). No template defect existed; the one real finding alongside it is WTV-040.
