...
### [WTV-046] Lead Vault appends landed at column AF — an open `A:AI` append range let Sheets pick the wrong table

- **Date:** 2026-07-29
- **Severity:** Blocker (a live lead was written where no downstream reader looks)
- **Symptom / Finding:** A production lead appended successfully but landed at **column AF** instead of column A. Every downstream consumer reads `A:L` or `A:V`, so the row is invisible: dedupe cannot see it, the retry tooling cannot find it, and a human scanning the sheet sees an empty row.
- **Root cause, in two layers:**
  1. **Why the write went sideways.** `appendLeadVault` used the range `All Leads!A:AI`. Google's `values.append` treats the supplied range as *a region in which to search for a table*, then appends after that table's last row **starting at that table's first column** — not necessarily at column A. Given a span reaching to AI, a stray data block in the right-hand columns is a valid "table", and Sheets aligned the write to it.
  2. **Why a stray right-hand block existed at all.** `leadRow()` emits **35 columns (A:AI)** while the sheet's canonical header is **12 columns (A:L)**. Every append had been writing 23 columns past the header — `M:AI`: `landing_page_url, referrer_url, utm_source, utm_medium, utm_campaign, utm_content, utm_term, fbclid, gclid, msclkid, product_name, product_slug, product_id, product_category, product_page_url, inventory_status, available_quantity, inventory_status_tag, lead_source, campaign, model_interest_tag, form_intent, timestamp`. Those orphaned columns accumulated into exactly the block that later captured the append. The layout drift was self-inflicted and gradual, which is why nothing caught it: each individual write succeeded.
  A third inconsistency compounds it: the dedupe read uses `A:V` (22 columns), so the code itself holds three different opinions about the table width — 12 in the sheet, 22 in the reader, 35 in the writer.
- **Fix (code):** appends are anchored at **`'<tab>'!A1`** via a shared `appendAnchored()` helper, for both `All Leads` and `Missed Leads`. Anchoring the search at A1 pins detection to the table containing A1, so a row can only ever start at column A. The helper also **asserts the result**: Sheets reports `updates.updatedRange`, and any range not beginning with column A now logs `LEAD VAULT LAYOUT DRIFT` with the offending range. A drifted sheet is client data this code must not silently rewrite — but it will never fail invisibly again.
- **Fix (data — owner-authorized, NOT done by code):** the sheet header must be widened to the 35 columns the writer actually produces, and the orphaned `AF:BI` block normalized or archived. Canonical header for both `All Leads!A1` and `Missed Leads!A1`:
  `submission_id, timestamp_iso, source, full_name, email, phone, financing_interest, page_url, ghl_status, ghl_contact_id, ghl_error, traffic_channel, landing_page_url, referrer_url, utm_source, utm_medium, utm_campaign, utm_content, utm_term, fbclid, gclid, msclkid, product_name, product_slug, product_id, product_category, product_page_url, inventory_status, available_quantity, inventory_status_tag, lead_source, campaign, model_interest_tag, form_intent, timestamp`
- **Rule / prevention:** never pass a multi-column span to `values.append`. Anchor at `!A1` and assert the returned `updatedRange`. Any change to `leadRow()`'s column count is a **sheet schema change** and must ship with the header update — the two cannot drift independently, because Sheets will not complain.
- **Status:** Code FIXED in `premium-redesign`. Sheet repair pending owner authorization.

---

### [WTV-047] A secret cannot be verified from outside the deployment — control-plane "configured" is not evidence

- **Date:** 2026-07-29
- **Severity:** Process blocker (root cause of WTV-045's lost day)
- **Symptom / Finding:** Production `GHL_API_TOKEN` and `GHL_LOCATION_ID` were declared as `secret_text` bindings and reported as configured by every available check, while the Function read both as empty strings. A real lead reached the Lead Vault with `ghl_error: GHL not configured` and no CRM contact.
- **Root cause:** the Cloudflare API returns `"value": ""` for **every** `secret_text` binding, including ones that demonstrably work — verified against `ADMIN_PASSWORD` and `META_PIXEL_ID` on this same project. A populated secret and a blank one are therefore **indistinguishable from outside the deployment**. Every party inspected the control plane, correctly saw the key present, and reported "configured". Nobody was wrong; the method could not produce the answer. The post-mortem confirmed no party retained the working token and Cloudflare's accessible audit data cannot attribute a Pages secret write, so the original blanking is unattributable — which is precisely why the check has to be structural rather than procedural.
- **Likely blanking mechanism:** a pipeline of the form `echo "$TOKEN" | wrangler pages secret put TOKEN` writes an **empty** secret and exits 0 when `$TOKEN` is unset in that shell. Silent success, no error to report. The same provisioning pass previously produced a Preview variable literally named `GHL_API_TOKEN ` with a trailing space in the key.
...
### [WTV-051] Build-time pixel injection hid a completely dead runtime CAPI config

- **Date:** 2026-07-29
- **Severity:** High (server-side conversions silently absent since first deploy)
- **Symptom / Finding:** The first authenticated runtime readiness report on `sun-pool-spa` showed `META_CAPI_ACCESS_TOKEN` and `META_PIXEL_ID` **empty** in both Production and Preview, and `META_OFFLINE_WEBHOOK_SECRET` empty. Server-side Conversions API had therefore never functioned on this project — while every tracking verification performed during the build reported Meta as working.
- **Root cause of the blind spot:** the two paths read the pixel ID from different places. The browser pixel is written into every page at **build time** by `injectMetaPixel()` from the client's `tokens.env`, so it initializes correctly and Pixel Helper shows a clean PageView regardless of runtime configuration. Server-side CAPI reads `env.META_PIXEL_ID` and `env.META_CAPI_ACCESS_TOKEN` at **request time**. Every check that had been run — view-source for `fbq('init', …)`, Pixel Helper, GA4 Realtime — exercised only the build-time path. The hardening that made the pixel robust (TVD: hardcode at build, independent of `client.config.js`) also removed the last signal that would have exposed an empty runtime config.
...
### [WTV-057] Seed products with invented prices were live on a client site, and no gate looks at D1 content

- **Date:** 2026-07-29
- **Severity:** High (published fabricated prices under the client's name)
- **Found on:** Sun Pool, while loading the client's real inventory. Production D1 held seven `available` products — `hydropool-self-cleaning-495` at `$7,995`, `hydropool-swim-spa-14fx` at `$24,995`, two saunas, and three more — each with a price, a monthly payment, and `featured` flags. Four were reachable in the public `/api/inventory` response and rendered on category pages.
- **Where they came from:** nowhere legitimate. `rg` across the client's `intake.json` and `tokens.env` matched none of the seven model names, and none appear in any template fixture or seed SQL. They were typed straight into remote D1 during the build to make the inventory renderer look populated, then never removed.
- **Why nothing caught it:** every existing check reads *files*. `gate.mjs` and `scan-placeholders.mjs` scan built HTML, `launch-check.mjs` reads `wrangler.toml` tokens, and `verify-product-route.mjs` proves a route renders — which it does, faithfully, using fabricated data. `secrets:verify` inspects bindings, not rows. D1 content has never been compared against the client's intake, so the one surface holding the site's factual claims about price and stock is the one surface no gate reads.
- **Why it matters more than a cosmetic bug:** the no-invented-facts law exists because these values reach `Product` JSON-LD with `offers.price`, where Google treats them as factual claims by the dealer. A wrong price on a real dealer's site is a consumer-facing misrepresentation, not a placeholder. It also survived a full launch-checklist pass and a click-through audit of the live site, because both were looking at whether things rendered, not whether the data was real.
