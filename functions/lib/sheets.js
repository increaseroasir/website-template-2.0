...
export async function appendLeadVault(env, lead, status, contactId, error) { return sheetsRequest(env, '/values/' + encodeURIComponent('All Leads!A:AI') + ':append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS', { method: 'POST', body: JSON.stringify({ values: [leadRow(lead, status, contactId, error)] }) }); }
...
export async function appendMissedLead(env, lead, error) { return sheetsRequest(env, '/values/' + encodeURIComponent('Missed Leads!A:AI') + ':append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS', { method: 'POST', body: JSON.stringify({ values: [leadRow(lead, 'FAILED', '', error)] }) }); }
