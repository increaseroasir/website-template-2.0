# Proposed migration: `onboarding_employees`

**Status:** PROPOSAL ONLY — do not apply  
**Target if later approved:** `htl-factory-dev` / `epeddfdifckzzmskhdsz`  
**Product form:** `employee_crm_access` (not storage `form2`)

When authorized, copy the SQL below into a dated file under `supabase/migrations/` and apply to **dev only**.
...
| Rule | Purpose |
|---|---|
| `UNIQUE (client_id, email_normalized)` | One employee row per client email |
| `UNIQUE (onboarding_case_id, source_submission_id)` | Replay-safe submissions |
| `payload` jsonb | Full raw product-form body preserved |

## Tests required before apply

- Insert + conflict on same email_normalized
- Replay same `source_submission_id` is no-op / unique violation handled
- No write path maps this table to `intake_submissions.form='form2'`
