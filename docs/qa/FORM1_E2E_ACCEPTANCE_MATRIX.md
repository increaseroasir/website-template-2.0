...
**Current status:** E2E-01/02/03/04/05/06/08 PASS; E2E-07 WAIVED; E2E-09 FAIL (unwrap re-verify 20260801T230505Z — Make `parseJSON` not found on LINK). See `artifacts/agent-runs/integrator/20260801T230505Z-form1-config-unwrap-fix-e2e09.md` (+ `20260801T223500Z-…`, `20260801T224600Z-…`). Make/Supabase synthetic acceptance **not closed**.  
...
| E2E-09 | **null_does_not_clear** | Link/create then second Form1 with null reported fields (new submission_id) | merge success; prior reported retained | intake append; config merge without clears | wipe Form2 ops; clear reported via null | **FAIL** | unwrap re-verify: CREATE `809589b2…` ok (`jsonb_typeof=string`); LINK `05023545…` HTTP 500 — `Function 'parseJSON' not found!`; no second patch |
| E2E-10 | **cleanup** | Delete/mark synthetic clients/cases/intakes/config for suite IDs | N/A | cleanup only | leave active schedule; leave orphan real data | **PASS** (zero residue after 20260801T230505Z E2E-09 run) | `live_verified_readonly` |
